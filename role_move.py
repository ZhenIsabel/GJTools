...
# fix:纠正切线后路线疯狂偏移
def move_map_correct(origin_coordinate, row_tag, width, height, try_times=2):
    ...
    if not current_loc == None:
        ...
        if 3 > offset[0] > -3 and 3 > offset[1] > -3:
            ...
            time.sleep(2)
    if try_times > 0:
        return move_map_correct(origin_coordinate, row_tag, width, height, try_times-1)
...
def move_map(width, height, callback_fun=None, origin=None):
    ...
    while y < height:
        ...
        if not origin == None and y > height/2:
            move_map_correct(origin, row_tag, width,height)
            count += callback_fun()
