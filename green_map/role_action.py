...
def buy_map():
    ...
    buy_count = cfg.buy_map_count
    if cfg.is_extra_buy:
        buy_count = buy_count-1
    buy_count_string = str(cfg.buy_count)
    if max_val > fitness_threshold:
...
def open_map():
    ...
    if max_val_count_check < 0.98:
        ...
        _, max_val_count_check, _, _ = cv2.minMaxLoc(
            matxh_res_800)
    if max_val_count_check < 0.98 and cfg.is_extra_buy:
        extra_buy_count = 10
    ...
    if max_val < fitness_threshold:
        ...
        i = 0
        pre_rest = 1
        while i < cfg.buy_map_count+extra_buy_count:
            time.sleep(cfg.single_open_time-pre_rest)
            i = i+1
            pre_rest = 0
            # 移除不对劲的buff
            remove_buff()
        ...
        pyautogui.moveRel(0, -100)
        up_horse()
        return extra_buy_count+cfg.buy_map_count
    else:
        close_dialog()
        up_horse()
...
def find_boxs():
    ...
    count += role_move.move_map(cfg.find_area_1[0],
                                cfg.find_area_1[1], find_box.find_box_under_footer)
    ...
    start_time = time.time()
    count += role_move.move_map(cfg.find_area_2[0],
                                cfg.find_area_2[1], find_box.find_box_under_footer,
                                cfg.begin_find_loc_2
                                )
...
def clear_bag():
    ...
    for j in range(0, 3):
        for i in range(0, cfg.bag_width):
            pyautogui.moveTo(
                first_loc[0] + i * cfg.bag_item_size, first_loc[1] + j * cfg.bag_item_size)
            pyautogui.rightClick()
    # for i in range(0, 10):
